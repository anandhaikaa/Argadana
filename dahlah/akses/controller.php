...
$query_servis = "SELECT * FROM macam_kategori";
...
if (!empty($_GET['ids'])) {
  $ids = $_GET['ids'];
  $queryedit_servis = "SELECT * FROM macam_kategori WHERE kode_barang_atau_layanan = '$ids' order by kode_barang_atau_layanan asc";
...
if ((!empty($_GET['tdr'])) OR (!empty($_POST['tdr']))) {
  ...
  $querydetail_reservasi = "SELECT * FROM transaksi_detail INNER JOIN macam_kategori ON `transaksi_detail`.`kode_barang_atau_layanan` = `macam_kategori`.`kode_barang_atau_layanan` WHERE `transaksi_detail`.`kode_transaksi` = '$tdr' order by kode_transaksi asc";
